...
foreach my $folder(@folder){
    chomp($folder);
    next if(!-d "$folder");
    next if(!-e "$folder/replicates/run1/model/model.bestlhoods");
    open(I,"< $folder/replicates/run1/model.est");
    my $num=0;
    my $light=-1;
    while (<I>) {
        chomp;
    ...
    my @bestlhoods=qx(ls $folder/replicates/run*/model/model.bestlhoods);
    my %hash;
    my $i=1;
    ...
    my $aic=2*$num-2*( $hash{$a[0]}{lhoods});
